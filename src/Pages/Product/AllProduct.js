...
const AllProducts = () => {
  ...
  const handleDiscount = (min, max) => {
    ...
    const isCurrentlySelected =
      selectedDiscount && selectedDiscount.min === min;
    ...
    if (isCurrentlySelected) {
      ...
      DiscountService.getFilteredProductFromDiscount(min, max).then(
        (response) => {
  ...
  const handleSortChange = (event) => {
    ...
    // Call your sorting functions based on selection
  ...
  return (
    <div className="container ">
      <div className="flex w-full">
        {/* Sidebar */}
        <div className=" xs:w-1/6 sm:w-2/4 md:w-2/5 lg:w-1/5 h-screen overflow-y-auto p-4">
          <div className="flex mb-4 mx-4 justify-between">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-left pr-10">
              Filters
            </h2>
            <button className="text-sm sm:text-base md:text-lg">Clear</button>
          </div>

          <div className="text-lg text-left mx-4 my-4 md:my-8">
            <h3 className="mb-2 text-lg sm:text-xl md:text-2xl">Price</h3>
            <div className="flex items-center mb-2">
              <input type="checkbox" id="shampoo" className="mr-2 h-4 w-4" />
              <label
                htmlFor="shampoo"
            ...
            <div className="flex items-center mb-2">
              <input type="checkbox" id="soap" className="mr-2 h-4 w-4" />
              <label htmlFor="soap" className="text-sm sm:text-base md:text-lg">
                150-200
              </label>
            </div>
            <div className="flex items-center mb-2">
              <input type="checkbox" id="hair-care" className="mr-2 h-4 w-4" />
              <label
                htmlFor="hair-care"
                className="text-sm sm:text-base md:text-lg"
              >
          ...
          <div className="text-lg text-left mx-4 my-4 md:my-8">
            <h3 className="mb-2 text-lg sm:text-xl md:text-2xl">Discount</h3>
            {discounts.map((discount, index) => (
              <div className="flex items-center mb-2" key={index}>
                <input
                  ...
                  checked={
                    selectedDiscount && selectedDiscount.min === discount.min
                  }
                  onChange={() => handleDiscount(discount.min, discount.max)}
                />
                <label
        ...
        {/* Products Grid */}
        <div className="w-5/6 md:w-5/5 p-4 h-screen overflow-y-auto">
          <div
            className="p-3 rounded-lg shadow-md w-full sm:w-[95%] mx-3"
            style={{ backgroundColor: "#DFF8FF" }}
          >
            <h1 className="text-xl font-semibold text-gray-800 dark:text-gray-200 text-left">
              TRESEmme
            </h1>
            <div className="flex flex-col sm:flex-row justify-between items-center">
              ...
              <div className="mt-4 flex flex-col sm:flex-row items-center sm:mt-0">
                <span className="text-gray-600 dark:text-gray-400">
                  Sort by:
                </span>

                {/* Dropdown for xs, sm, and md screens */}
                <select
                  value={selectedSort}
                  onChange={handleSortChange}
                  className="mt-2 border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 sm:block md:block lg:hidden"
                >
                ...
                {/* Sort buttons for lg and larger screens */}
                <div className="hidden lg:flex mt-2 sm:mt-0">
                  <button
                    className="ml-2 px-4 py-2 text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700"
                    onClick={() =>
                  ...
                  <button
                    className="ml-2 px-4 py-2 flex text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700"
                    onClick={() =>
                      handleSortChange({ target: { value: "High to Low" } })
                    }
                  >
                    High to Low
                  ...
                  <button
                    className="ml-2 px-4 py-2 flex text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700"
                    onClick={() =>
                      handleSortChange({ target: { value: "Low to High" } })
                    }
                  >
                    Low to High
                    <div className="w-6 ml-1">
              ...
              {product.map((product) => (
                ...
                >
                  <div className="bg-white p-4 shadow relative">
                    <img
                      className="w-3/4 h-auto mx-auto mt-10"
                      ...
                      alt={product.medicine.medicineName}
                    />
                    {product.discount && (
